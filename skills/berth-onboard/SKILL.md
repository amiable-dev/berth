...
3. **Give the checkout its numbers.**
   ...
   - Docker Compose with hardcoded host ports: first `berth who <port>` on each port the compose file publishes. If every holder is either unbound or a container `berth who` shows as `started by: compose <project>`, run `eval "$(berth env --compose-override)"` and show the human the mapping it printed (service → new host port); `berth doctor` names the compose command this machine has (`docker compose` or `docker-compose`) for the up step that follows. If a holder is `started by: docker run`, the override cannot move it — Compose does not know that container exists. Show the human the recreate command `berth env --compose-override` prints instead (stop, remove, `docker run` again on the new port, carrying its volumes) and get their go-ahead before running it; read out any one-line caution it prints (an anonymous volume, or state the image keeps only in memory) rather than running past it.
